...
export default {
  ...
  mounted() {
    if (window.kakao && window.kakao.maps) {
      ...
      script.src =
        "//dapi.kakao.com/v2/maps/sdk.js?autoload=false&appkey=212c3ad52c21a8156fe970aa01882e45";
    ...
    addClusterer() {
      setTimeout(() => {
        if (this.viewUserNickName) {
          ...
          for (let i = 0; i < this.viewUserTravelList.length; i++) {
            for (
              let j = 0;
              j < this.viewUserTravelList[i].pinList.length;
              j++
            ) {
              var x = this.viewUserTravelList[i].pinList[j].location.x;
              var y = this.viewUserTravelList[i].pinList[j].location.y;
              ...
              var marker = new kakao.maps.Marker({
                ...
                title:
                  this.viewUserTravelList[i].pinList[j].location.place_name, // 마커의 타이틀, 마커에 마우스를 올리면 타이틀이 표시됩니다
              ...
              var basicInfo =
                `<div class="customoverlay">
                            <a href="` +
                this.viewUserTravelList[i].pinList[j].location.place_url +
                `" 
                            target="_blank">
                            <span class="title">` +
                this.viewUserTravelList[i].pinList[j].location.place_name +
                `</span>
                            </a>
                            </div>`;
              ...
              var moreInfo =
                ...
                this.viewUserTravelList[i].pinList[j].images[0].base64Image +
                `');
                ...
                `
                                    ...
                                    <div class="subtitle">
                                        ` +
                this.viewUserTravelList[i].pinList[j].location
                  .category_group_name +
                `
                                    </div>          
                                </div>
                                <div class="address">
                                    ` +
                this.viewUserTravelList[i].pinList[j].location.address_name +
                `
                                </div>
                            </div>
                            </div>
  ...
  computed: {
    ...mapGetters(["viewUserNickName", "viewUserTravelList"]),
  },
};
</script>
<style scoped>
